// Language switching (same as login/register)
...
resetForm.addEventListener('submit', function(e) {
  ...
  if (!newPassword || !confirmPassword) {
    alert(currentLang === 'ar' ? 'يرجى إدخال كلمة المرور وتأكيدها' : 'Please enter and confirm your password');
    return;
  }
  if (newPassword.length < 6) {
    alert(currentLang === 'ar' ? 'كلمة المرور يجب أن تكون 6 أحرف على الأقل' : 'Password must be at least 6 characters long');
    return;
  }
  if (newPassword !== confirmPassword) {
  ...
  alert(currentLang === 'ar' ? 'تمت إعادة تعيين كلمة المرور بنجاح!' : 'Password reset successfully!');
}); 