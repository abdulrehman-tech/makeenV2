...
function copyTableToClipboard() {
  const table = document.querySelector('.programs-table');
...
function exportToCSV() {
  ...
  downloadFile(csvContent, 'programs.csv', 'text/csv');
...
function printTable() {
  ...
  const table = document.querySelector('.programs-table').cloneNode(true);
...
const statusActions = {
  pending: ['preview', 'edit', 'publish'],
  published: ['preview', 'program_requests'],
  rejected: ['preview'],
  closed: ['preview', 'program_requests'],
  draft: ['preview', 'edit', 'publish']
};
...
function handleActionItemClick(e) {
  ...
  switch(action) {
    case 'preview':
      showNotification(`Previewing ${programName}`, 'info');
      break;
    case 'edit':
      showNotification(`Editing ${programName}`, 'info');
      break;
    case 'program_requests':
      showNotification(`Viewing requests for ${programName}`, 'info');
      break;
    case 'publish':
...
function updateProgramStatus(row, newStatus) {
  ...
  if (statusBadge && dropdown) {
    ...
    showNotification(`Program status updated to ${getTranslation(newStatus)}`, 'success');
  }
}
